...
export async function GET(req: NextRequest) {
  ...
  if (!TM_KEY) {
    return NextResponse.json({ events: [], city: null });
  }
  ...
  url.searchParams.set("endDateTime", in14Days.toISOString().slice(0, 19) + "Z");
  url.searchParams.set("size", "10");
  url.searchParams.set("sort", "date,asc");

  // Also reverse-geocode for city name
  ...
  try {
    ...
    const events: HomepageEvent[] = tmEvents.slice(0, 3).map((ev) => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const bestImage = [...((ev.images ?? []) as any[])].sort(
        (a, b) => b.width * b.height - a.width * a.height
      )[0];

      const base: number = ev.priceRanges?.[0]?.min ?? 85;
